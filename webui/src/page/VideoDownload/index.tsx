import { CheckCircleFilled, CloudDownloadOutlined, ExclamationCircleFilled, LinkOutlined, PlayCircleFilled, ThunderboltOutlined } from '@ant-design/icons'
...
export default function VideoDownload() {
  ...
  const parse = async () => {
    ...
    setLoading(true)
    try {
      const nextResult = await getVideoInfo(url.trim())
  ...
  const download = async () => {
    if (!result) return
    setDownloading(true)
    try { const file = await downloadVideo({ url: result.webpage_url || url.trim(), platform: result.platform, format: 'mp4' }); triggerFileDownload(file); message.success('视频下载已开始') } catch (error) { message.error(error instanceof Error ? error.message : '视频下载失败') } finally { setDownloading(false) }
  }
